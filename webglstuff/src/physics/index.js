...
export function initPhysics(heightfield, boostersParam) {
    boosters = boostersParam;
...
export function resetPhysics(textureParam) {
    ...
    for (const booster of boosters) {
        ...
        const component = imuplseColorComponents[booster.type];

        if (booster.mesh)  {
            booster.mesh.visible = true;

            const scale = 0.5 + colorMix[component] * 1.0;
            booster.mesh.scale.set(scale, scale, scale);
        }

        booster.strength = colorMix[component];
    }
...
export function updatePhysics(dtSeconds) {
    ...
    const boosterClosenessLimit = scale * 1.99;

    const imuplseStrengths = {
        "upper": new CANNON.Vec3(4, 12, 0),
        "speeder": new CANNON.Vec3(8, 4, 0),
        "downer": new CANNON.Vec3(12, -4, 0),
    }

    const bodiesCenterAdjusted = bodiesCenter.clone();

    for (const booster of boosters) {
        const distance = booster.mesh.position.distanceTo(bodiesCenterAdjusted);
        if (!booster.used && distance < boosterClosenessLimit) {
            booster.used = true;
            
            const strength = imuplseStrengths[booster.type];            
            strength.mult(booster.strength * 1.25, strength);

            bodies.map((body) => {
                const impulseLocation = body.position;
                
                body.applyImpulse(strength, impulseLocation);
            });
            console.log("Impulse!", booster.type, booster.strength, booster);
        } 
    }
